...
def run_pipeline(data_df, open_smile_feature_count, run_name_prefix, run_tags, create_df_artifact) -> None:
...
def main(experiment_name, open_smile_feature_count, create_df_artifact, num_processes, save_path):
    ...
    print("\ncreate_df_artifact: \n", create_df_artifact)
    print("\nnum_processes: \n", num_processes)
    print("\nusing {} processes out of {} available processes: \n".format(num_processes,multiprocessing.cpu_count()))
    ...
    for fake_speaker_chunk in fake_speaker_partitions:
        for real_speaker_chunk in real_speaker_partitions:
            ...
            data_df = timit_data_loader.generate_split_speaker_test(voices_to_remove, folder=False)
            ...
            args = (data_df, open_smile_feature_count, run_name_prefix, run_tags, create_df_artifact)
    ...
    agg_results = runs.groupby(['tags.feature_method', 'tags.estimator_name', 'tags.label_type'])['metrics.accuracy', 'metrics.0_accuracy', 'metrics.1_accuracy', 'metrics.eer_score'].mean().reset_index()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default='results_multivoice.csv', help="Path of the CSV file to save")
    ...
    main(experiment_name, open_smile_feature_count, create_df_artifact, num_processes, save_path)
