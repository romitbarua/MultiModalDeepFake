...
def truncate_silences(normalized_audio, window_size, silence_threshold, sr=None, low_pass_filter_cutoff=None, counter=0):

    start_ids = []
    end_ids = []
    truncated_audio = []
    
    #go through each audio and truncate the silences
    for audio in normalized_audio:
        ...
        counter += 1
        if counter % 100 == 0:
            print(f'Truncating audio {counter}/{len(normalized_audio)} ({round(counter*100/len(normalized_audio))}%)')
...
def get_silence(audio, window_size, silence_threshold, sr = None, low_pass_filter_cutoff=None):
    
    #GK Comment: Revist this logic
    thresh = max(abs(audio))*silence_threshold

    moving_avg = moving_average(abs(audio), window_size)

    #This abs value is redundant
...
def get_silence_spread(audio, window_size, silence_threshold, sr=None, low_pass_filter_cutoff=None):
