...
class smileFeatureSelectFromModel(smileFeatureSelectorBase):
    ...
    def select_features_binary(self, max_features=10, return_df=False, print_features=True, return_features=False):
        """
        Selects the top num_features features based on the model specified
        """

        #for binary classification
        sfm_features = self._run_sfm(self.train_df, self.dev_df, max_features, multiclass=False)
