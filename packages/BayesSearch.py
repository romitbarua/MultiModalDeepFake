...
class BayesSearch:
    ...
    def __init__(self, data, target_function, sampling_function, n_iter, init_ex_count=20, gp_ex_count=1000):
        self.target_function = target_function
        self.gp_reg = GaussianProcessRegressor()
        self.output = pd.DataFrame(columns=['WindowSize', 'SilenceThreshold', 'Acc', 'EI']) ## SB_COmment - any reason for EI over UCB or PI (Upper Confidence Bound/Probability of Improvement)
    ...
    def expected_improvement(self, h_new):
        ...
        mean_y_new, sigma_y_new = self.gp_reg.predict(np.array([h_new]), return_std=True)
